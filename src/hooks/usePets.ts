...
import { SEED_PETS } from '../../Seeddata'
import type { Pet, SortOption, FilterMode, Inventory, Watchlist } from '../../index'
...
export function usePets({ sort, filter, search, page, inventory, watchlist }: UsePetsOptions) {
  ...
  let filtered = allPets.filter(p => p.name.toLowerCase().includes(search.toLowerCase()))
